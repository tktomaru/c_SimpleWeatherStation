...
return_type aes_set_key_zach( const unsigned char key[], length_type keylen, aes_context ctx[1] )
{
    uint_8t cc, rc, hi;
...
return_type aes_encrypt_zach( const unsigned char in[N_BLOCK], unsigned char  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
...
return_type aes_cbc_encrypt_zach( const unsigned char *in, unsigned char *out,
                         int n_block, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    {
        xor_block(iv, in);
        if(aes_encrypt_zach(iv, iv, ctx) != EXIT_SUCCESS)
			return EXIT_FAILURE;
...
return_type aes_decrypt_zach( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
        uint_8t s1[N_BLOCK], r;
...
return_type aes_cbc_decrypt_zach( const unsigned char *in, unsigned char *out,
                         int n_block, unsigned char iv[N_BLOCK], const aes_context ctx[1] )
{   
    while(n_block--)
    {   uint_8t tmp[N_BLOCK];
        ...
        if(aes_decrypt_zach(in, out, ctx) != EXIT_SUCCESS)
			return EXIT_FAILURE;
        xor_block(out, iv);
...
void aes_encrypt_128_zach( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK],
...
void aes_decrypt_128_zach( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK],
                      const unsigned char key[N_BLOCK], unsigned char o_key[N_BLOCK] )
{
...
void aes_encrypt_256_zach( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK],
...
/*  Decrypt a single block of 16 bytes with 'on the fly'
    256 bit keying
*/
void aes_decrypt_256_zach( const unsigned char in[N_BLOCK], unsigned char out[N_BLOCK],
                      const unsigned char key[2 * N_BLOCK], unsigned char o_key[2 * N_BLOCK] )
{
